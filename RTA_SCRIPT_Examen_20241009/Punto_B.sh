...
for (( i=1; i<=3; i++ )); do
    ...
    for (( j=1; j<=3; j++ )); do
        ...
        echo "${DISCO}${CONTADORDISCO}  /home/vagrant/Examenes-UTN/alumno_${i}/parcial_${j}   ext4  defaults  0 0" | sudo tee -a /etc/fstab
        CONTADORDISCO=$((CONTADORDISCO + 1))  
    done
done
echo "${DISCO}${CONTADORDISCO}  /home/vagrant/Examenes-UTN/profesores  ext4  defaults  0 0" | sudo tee -a /etc/fstab
